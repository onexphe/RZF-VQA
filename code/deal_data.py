...
import Levenshtein
import wordninja
from data import fvqa, preprocess,fvqa_glove_fact
import random
import numpy as np
from collections import defaultdict
...
class Runner:
    ...
    def preprocess_dict(self):
        output_format = osp.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.entity.500.json")
        if not osp.exists(output_format):
            path = osp.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json")
            ...
            entities = fvqa.prepare_entity(annotations)
            entity_vocab = preprocess.extract_vocab(entities, top_k=None)
            vocabs = {'answer': entity_vocab}
            print('* Dump output vocab to: {}'.format(output_format))
            with open(output_format, 'w') as fd:
                json.dump(vocabs, fd)
        print("preprocess_dict done!")


    def preprocess_fact_surface(self):
        output_format = osp.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.fact_surface.500.json")
        if not osp.exists(output_format):
            num = 2
            vqa_train_questions = osp.join(self.args.FVQA.train_data_path, "train" + str(num), "all_qs_dict_release_train_500.json")
            vqa_val_questions = osp.join(self.args.FVQA.test_data_path, "test" + str(num), "all_qs_dict_release_test_500.json")
            ...
            annotations = {**qaq1, **qaq2}
            # word2vec = Vector()
            fact_surfaces = fvqa_glove_fact.prepare_fact_surface(annotations)
            fact_surface_vocab = preprocess.extract_vocab(fact_surfaces, top_k=None)
            vocabs = {'answer': fact_surface_vocab}
            print('* Dump output vocab to: {}'.format(output_format))
            with open(output_format, 'w') as fd:
                json.dump(vocabs, fd)
        print("preprocess_fact done!")
    ...
    def resort_dict(self):
        input_format = osp.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.entity.500.json")
        output_format = osp.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.entity.resort.500.json")
        with open(input_format, 'r') as fd:
            dict1 = json.load(fd)
        path = osp.join(self.args.FVQA.common_data_path, "answer.vocab.fvqa.500.json")
        with open(path, 'r') as fd:
            dict2 = json.load(fd)
        ...
        i = 500
        for one in dict1['answer'].keys():
            if one in entity_vocab['answer']:
                continue
            entity_vocab['answer'][one] = i
            i += 1
    ...
    def get_fact_relation_fact_matrix(self):
        if not osp.exists(self.args.FVQA.fact_relation_to_fact_path):
            ...
            self.index_to_entity = preprocess.invert_dict(self.entity_to_index)
            self.relation_to_index = relation_vocab['answer']
            self.index_to_relation = preprocess.invert_dict(self.relation_to_index)

            output_format = self.args.FVQA.fact_relation_to_fact_path
            vqa_questions = osp.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json")
            with open(vqa_questions, 'r') as fd:
                qaq1 = json.load(fd)

            annotations = {**qaq1}

            entity_num = len(self.entity_to_index)
            rel_num = len(self.relation_to_index)

            # fact_relation_matrix = - np.ones([fact_num,rel_num ], dtype = int)
            fact_relation_to_fact = defaultdict(list)
            ...
            for a in keys:
                ...
                f1 = preprocess.process_punctuation(f1)
                f2 = preprocess.process_punctuation(f2)
                rel = preprocess.process_punctuation(rel)
                ...
                name = str(self.entity_to_index[fact]) + "-" + str(self.relation_to_index[rel])
                if not self.entity_to_index[answer] in fact_relation_to_fact[name]:
                    fact_relation_to_fact[name].append(self.entity_to_index[answer])


                # name = str(self.entity_to_index[f1]) + "-" + str(self.relation_to_index[rel])
                # if not self.entity_to_index[answer] in fact_relation_to_fact[name]:
                #     fact_relation_to_fact[name].append(self.entity_to_index[answer])
                # name = str(self.entity_to_index[f2]) + "-" + str(self.relation_to_index[rel])
                # if not self.entity_to_index[answer] in fact_relation_to_fact[name]:
                #     fact_relation_to_fact[name].append(self.entity_to_index[answer])
            with open(output_format, 'w') as fd:
                json.dump(fact_relation_to_fact, fd)
                print("dump done!")

    def get_fact_matrix(self):
        if not osp.exists(self.args.FVQA.fact_path):
            ...
            with open(fact_surface_vocab_path, 'r') as fd:
                fact_surface_vocab = json.load(fd)

            self.answer_to_index = answer_vocab['answer']
            self.index_to_answer = preprocess.invert_dict(self.answer_to_index)
            self.entity_to_index = fact_vocab['answer']
            self.index_to_entity = preprocess.invert_dict(self.entity_to_index)
            self.relation_to_index = relation_vocab['answer']
            self.index_to_relation = preprocess.invert_dict(self.relation_to_index)
            self.fact_surface_to_index = fact_surface_vocab['answer']
            self.index_to_fact_surface = preprocess.invert_dict(self.fact_surface_to_index)

            output_format = self.args.FVQA.fact_path
            vqa_questions = osp.join(self.data_path, "all_qs_dict_release_combine_filter_fusion_500.json")
            with open(vqa_questions, 'r') as fd:
                qaq1 = json.load(fd)
            ...
            for a in keys:
                answer = annotations[a]["answer"]
                facts = annotations[a]["fact"]
                fact_surface =  annotations[a]["fact_surface"]
                f1 = facts[0]
                rel = facts[1]
                f2 = facts[2]
                assert (answer == f1 or answer == f2)
                ...
                f2 = preprocess.process_punctuation(f2)
                rel = preprocess.process_punctuation(rel)
                fact_surface = preprocess.process_punctuation(fact_surface)
                ...
                fact_matrix[self.fact_surface_to_index[fact_surface]] = [self.entity_to_index[fact],self.relation_to_index[rel],self.answer_to_index[answer]]
            with open(output_format, 'w') as fd:
                json.dump(fact_matrix, fd)
                print("dump done!")
...
if __name__ == '__main__':
    ...
    # runner.preprocess_dict()
    # runner.resort_dict()
    # runner.get_fact_relation_fact_matrix()
    # runner.preprocess_fact_surface()
    runner.get_fact_matrix()
