...
from data import fvqa_glove_fact as fvqa
import json
...
class Runner:
    def __init__(self, args):
        ...
        self.avocab_fact_surface = default_collate(list(range(0, 3913)))
        ...
        self.fusion_model_fact_surface, self.answer_net_fact_surface = self._model_choice(args)
        ...
        self._load_model(self.fusion_model_fact_surface, "fusion", "fact")
        self._load_model(self.answer_net_fact_surface, "embedding", "fact")
    ...
    def run(self):
        ...
        tmp_args.relation_map = 0
        tmp_args.fact_surface_map = 1
        self.train_loader_fact_surface = fvqa.get_loader(tmp_args, self.word2vec, train=True)
        ...
        for method_choice in self.method_list:
            ...
            answer_var_fact_surface, _ = self.train_loader_fact_surface.dataset._get_answer_vectors(method_choice, self.avocab_fact_surface)
            ...
            answer_var_fact_surface = F.normalize(answer_var_fact_surface, p=2, dim=1)
        ...
        self.answer_var_fact_surface = Variable(answer_var_fact_surface.float()).cuda()
    ...
    def eval(self):
        ...
        self.fusion_model_fact_surface.eval()
        self.answer_net_fact_surface.eval()

        tq = tqdm(self.val_loader)
        fact_matrix = self._get_fact_matrix()
        ...
        for visual_features, boxes, question_features, answers, idx, q_len in tq:
            with torch.no_grad():
                ...
                fusion_embedading_fact_surface = self.fusion_model_fact_surface(visual_features, boxes, question_features, q_len)
                answer_embedding_fact_surface = self.answer_net_fact_surface(self.answer_var_fact_surface)
                ...
                predicts_fact_surface = cosine_sim(fusion_embedading_fact_surface, answer_embedding_fact_surface) / self.args.loss_temperature  # temperature 与每一个答案都有计算相似度，
                predicts_fact_surface = predicts_fact_surface .to(torch.float64)               

                self.get_min_and_max(predicts_ans.data)

                self.val_metrics_orig.update_per_batch(0, answers.data, predicts_ans.data)
#                 if self.temp:
#                     print('pre',predicts_ans[0])
                ...
                if self.args.joint_test_way == 0:  # way 0
                    # mask_matrix = self._get_mask_on_ans(predicts_ans,predicts_rel, predicts_fact, fact_relation_to_ans)

                    # # add mask
                    # predicts_ans = predicts_ans + mask_matrix

                    predicts_ans = self._get_predict_ans(predicts_ans,predicts_rel, predicts_fact, predicts_fact_surface,fact_matrix)
    ...
    def explain_experiments(self, predicts_rel, predicts_fact, predicts_ans, predicts_ans_orig, answers, fact_relation_to_ans, idx):
        ...
        if True or self.val_metrics.correct_1 >= self.val_metrics_orig.correct_1 and self.val_metrics.correct_1 > 0:
            # get the top 10 ans
            ...
            ida = idx
            for it in range(predicts_rel.shape[0]):
                pre_h_r = []
                pre_t = []
                _, top_rel = predicts_rel.topk(self.args.top_rel, dim=1)
                _, top_fact = predicts_fact.topk(self.args.top_fact, dim=1)
                ...
                if len(answers.data.shape) == 3:
                    answers = answers.data[it]
                _, real_ans = answers.topk(1, dim=1)

                top_rel = top_rel[it].cpu().numpy().tolist()
                top_fact = top_fact[it].cpu().numpy().tolist()
                pre_ans = pre_ans[it].cpu().numpy().tolist()
                pre_ans_orig = pre_ans_orig[it].cpu().numpy().tolist()
                idx = ida[it].item()
                real_ans = real_ans[it][0].item()
                ...
                self.result[str(idx)] = [idx,real_ans,pre_ans_real,top_rel_real,top_fact_real,pre_h_r,pre_t]
                ...
                if False or real_ans not in pre_t and len(pre_t) >= 1:
                    logger.info('#################################################################################')
    ...
    def _load_model(self, model, function, type_name):
        ...
        assert type_name in ["answer", "relation", "fact","fact_surface"]
        target = type_name
    ...
    def _get_fact_matrix(self):
        
        with open(self.args.FVQA.fact_path, 'r') as fd:
            fact_relation_to_ans = json.load(fd)
        return fact_relation_to_ans


    def _get_mask_on_ans(self,predicts_ans, predicts_rel, predicts_fact, fact_relation_to_ans):
        # self.train_loader.dataset 必须是relation和fact，这样才能映射到relation和fact上。不然都是 128 * 500
        mask_ans = torch.zeros(predicts_rel.shape[0], self.args.FVQA.max_ans)
                ...
                for j in range(self.args.top_fact):
                    ...
                    if name in fact_relation_to_ans.keys():
                        for k in fact_relation_to_ans[name]:
                            ...
                            mask_ans[item][k] = predicts_rel[item][top_rel[item][i].item()].item()+predicts_fact[item][top_fact[item][j].item()]
                            
        
        
        for i in range(predicts_rel.shape[0]):       
            mask_ans[i,:] = MaxMinNormalization(mask_ans[i,:])
        mask_ans = mask_ans.cuda()
        if self.temp:
            print('mask',mask_ans[0])

        mask_ans = mask_ans * 1 * self.args.soft_score
        
        if self.temp:
            print('mask',predicts_ans[0]+mask_ans[0])
            self.temp = False
#         mask_ans = mask_ans.cuda()
        ##############################################################################
        return mask_ans

    def _get_predict_ans(self, predicts_ans, predicts_rel, predicts_fact, predicts_fact_surface ,fact_matrix):
        fact_w = torch.zeros(predicts_rel.shape[0], 100)
        answer_w = torch.zeros(predicts_rel.shape[0], self.args.FVQA.max_ans)
...
#             input = torch.pow(input, 2)
        ...
        # 正规化
#         predicts_ans = MaxMinNormalization(predicts_ans)
#         predicts_rel = MaxMinNormalization(predicts_rel)
#         predicts_fact = MaxMinNormalization(predicts_fact)
#         predicts_fact_surface = MaxMinNormalization(predicts_fact_surface)
        predicts_ans = l2_norm(predicts_ans)
        predicts_rel = l2_norm(predicts_rel)
        predicts_fact = l2_norm(predicts_fact)
        predicts_fact_surface = l2_norm(predicts_fact_surface)
        
        
        # 把每一行的 predicts_rel 和 predicts_fact 中 top 拿出来凑对并且从fact_relation_to_ans筛选，将对应位置置0。rel top10，fact top30
        _, top_rel = predicts_rel.topk(3, dim=1)
        _, top_fact = predicts_fact.topk(10, dim=1)
        _, top_fact_surface = predicts_fact_surface.topk(100, dim=1)
        
        if self.temp:
            print(predicts_ans[0,:20])
#         for item in range(predicts_rel.shape[0]):
#             for i in range(predicts_rel.shape[1]):
#                 predicts_rel[item][top_rel[item][i].item()] = predicts_rel[item][top_rel[item][i].item()] / (1+i)
#         for item in range(predicts_fact.shape[0]):
#             for i in range(predicts_fact.shape[1]):
#                 predicts_fact[item][top_fact[item][i].item()] = predicts_fact[item][top_fact[item][i].item()] / (1+i)
                
        for item in range(top_fact_surface.shape[0]):
            for i in range(100):
                fact,rel,answer = fact_matrix[str(top_fact_surface[item][i].item())]
                status = [False,False]
                for j in range(3):
                    if top_rel[item][j] == rel:
                        status[1] = True
#                         predicts_rel[item][rel] = predicts_rel[item][rel].item() / (j+1)
                for j in range(10):
                    if top_fact[item][j] == fact:
                        status[0] = True
#                         predicts_fact[item][fact] = predicts_fact[item][fact].item() / (j+1)
                    
                
                score = predicts_rel[item][rel].item()+predicts_fact[item][fact].item()+predicts_fact_surface[item][top_fact_surface[item][i].item()]
                score = predicts_rel[item][rel].item()
                if answer_w[item][answer].item() < score and status[0] == True and status[1] == True:
                    answer_w[item][answer] = score

        predicts_ans = 0.1*answer_w.cuda() + predicts_ans.cuda()
        
        if self.temp:
            print(predicts_ans[0,:20])
            self.temp = False
        
        return predicts_ans.cuda()
