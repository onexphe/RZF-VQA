...
class Runner:
    def __init__(self, args):
        ...
        #get gcn model
        # Model and optimizer
        self.gcn_model = gcn.GCN(nfeat=505,
                    nhid=24,
                    nclass=500,
                    dropout=args.TRAIN.dropout).cuda()

        # optimizer
        self.optimizer = optim.Adam(self.gcn_model.parameters(),lr=args.TRAIN.lr)
    ...
    def train(self,epoch):
        ...
        fact_relation_to_fact = self._get_fact_relation_fact_dict()
        ...
        for visual_features, boxes, question_features, answers, idx, q_len in tq:
            ...
            # graph
            adj, features = self._get_graph(predicts_rel, predicts_fact, predicts_ans,fact_relation_to_fact)
            labels = answers
            loss_all = 0
            for i in range(labels.shape[0]):
                self.optimizer.zero_grad()
                output = self.gcn_model(features[i],adj[i])
                target = torch.tensor([labels[i].argmax()]).cuda()
                loss_all += F.nll_loss(output, target.long())

                # if self.itr % 1000 == 0:
                #     logger.info(f'#### loss={loss_all}{output},{target}')
                #     logger.info(f'#### size={output.size()},{target.size()}')
                #     logger.info(f'#### length={labels.shape[0]}')
                # self.itr += 1
            logger.info(f'#### loss={loss_all}{output},{target}')
            logger.info(f'#### size={output.size()},{target.size()}')
            logger.info(f'#### length={labels.shape[0]}')
        ...
        for visual_features, boxes, question_features, answers, idx, q_len in tq:
            with torch.no_grad():
                ...
                for i in range(labels.shape[0]):
                    ...
                    predict_lable = output.argmax()
                    target = torch.tensor([labels[i].argmax()]).cuda()
                    logger.info(f'#### Eval test={predict_lable},{labels[i].argmax()}')
                    if predict_lable == target:
                        self.correctNum += 1
                    self.allNum += 1
        logger.info(f'#### Eval top1={self.correctNum/self.allNum}')
    ...
    def _get_graph_v2(self, predicts_rel, predicts_fact, predicts_ans,fact_relation_to_fact):
        #  ([128, 500])([128, 103])([128, 2791])
        mask_ans = torch.ones(predicts_rel.shape[0], self.args.FVQA.max_ans) # [128,500]
            ...
            for i in range(self.args.top_rel):
                for j in range(self.args.top_fact):
                    ...
                    if name in fact_relation_to_fact.keys():
                        for k in fact_relation_to_fact[name]:
                            adj[item][self.args.FVQA.max_ans+j][k] = 1
                            graph[item][self.args.FVQA.max_ans+j][k] = i + 1
    ...
    def _get_graph_test(self, predicts_rel, predicts_fact, fact_relation_to_ans):
        #  ([128, 500])([128, 103])([128, 2791])
        mask_ans = torch.ones(predicts_rel.shape[0], self.args.FVQA.max_ans) # [128,500]
        ...
        graph = torch.zeros(predicts_rel.shape[0], self.args.FVQA.max_fact,self.args.FVQA.max_ans) #[128,2790/500,500]
        adj = torch.zeros(predicts_rel.shape[0],self.args.FVQA.max_fact,self.args.FVQA.max_ans) #[128,2790/500,500]
        ##############################################################################
        # TODO: optimize this process via relation matric mapping rather than “ for-loop”
        for item in range(predicts_rel.shape[0]):
            for i in range(self.args.top_rel):
                for j in range(self.args.top_fact):
                    ...
                    if name in fact_relation_to_ans.keys():
                        for k in fact_relation_to_ans[name]:
                            graph[item][top_fact[item][j].item()][k] = top_rel[item][i].item() + 1
                            adj[item][top_fact[item][j].item()][k] = 1
                            mask_ans[item][k] = 0
        ##############################################################################
