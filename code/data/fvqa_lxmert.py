...
class FVQA(VisualQA):  # ok
    ...
    def __init__(self, args, qa_path, vector, file_path=None):
        ...
        img_data = []
        img_data.extend(load_obj_tsv(args.FVQA.feature_obj36_path ))
        ...
        self.question_text = self._get_question_text()
        # pdb.set_trace()
        # process images 处理图片
    ...
    def open_hdf5(self):
        ...
        self.image_ids_oringin = self._get_img_id_oringin()
    ...
    def __getitem__(self, item):  # ok
        ...
        #     pdb.set_trace()
        question = self.question_text[item]
        question_f, question_length = self.questions[item]  # 问题向量列表
        # sample answers
        ...
        imageFeature, _ = self._load_image(self.image_ids_oringin[item])    # 直接获得图片的特征
        ...
        # pdb.set_trace()
        return imageFeature, feats, boxes, question,label, item, question_f,question_length
    ...
    def _get_question_text(self):
        questions_json = self.qa_json
        keys = list(questions_json.keys())
        questions = []
        for a in keys:
            questions.append(questions_json[a]['question'].lower()[:-1])  # question的list
        return questions
