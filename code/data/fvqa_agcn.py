...
from data.agcn.agcn_RE_processor import RE_Processor
...
class FVQA(VisualQA):  # ok
    ...
    def __init__(self, args, qa_path, vector, file_path=None):
        ...
        self.dep_features =  self.initAgcn()
    ...
    def __getitem__(self, item):  # ok
        ...
        #dep tree
        dep_matrix = self.dep_features[item]["dep_type_matrix"]

        # return  image, spa,question_vec, question, label, item,input_ids, segment_ids, attention_mask,question_length,dep_matrix
        return item,label,(image,spa),(question,question_vec,question_length),(input_ids,input_mask, segment_ids),dep_matrix
    ...
    def initAgcn(self):
        with open(os.path.join(self.args.agcn_path, "dict.json"),'r')as f:
            dict_bin = json.load(f)
        processor = RE_Processor(direct=True,dep_type='local_global_graph', types_dict=dict_bin["types_dict"], labels_dict=dict_bin["labels_dict"])
        examples = processor.get_train_examples(self.file_path)
        features = processor.convert_examples_to_features(examples, tokenizer=None, max_seq_length=self.max_seq_length)
        return features

    def _get_dep_metrix(self):
