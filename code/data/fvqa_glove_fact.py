...
class FVQA(VisualQA):  # ok
    ...
    def __init__(self, args, qa_path, vector, file_path=None):
        ...
        elif args.fact_surface_map:
            name = "fact_surface"
            self.answers = list(prepare_fact_surface(self.qa_json)) 
        else:
            name = "answer"
    ...
    def _get_fact_vectors(self, ways, answer_indices):
...
def prepare_fact_surface(answers_json):
    keys = list(answers_json.keys())
    fact_surfaces = []
    for a in keys:
        facts = answers_json[a]["fact_surface"]
        fact_surfaces.append([facts] * 10)  # 双层list，内层的list对应一个问题的答案序列
    for fact_surface in fact_surfaces:
        ret = list(map(process_punctuation, fact_surface))  # 去除标点等操作
        yield ret
