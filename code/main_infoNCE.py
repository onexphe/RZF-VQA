    ...
    def train(self, epoch):
        ...
        for visual_features, boxes, question_features, answers, idx, q_len in tq:
            ...
            # Mapping-based methods
                ...
                predicts = cosine_sim(fusion_embedading, answer_embedding) / self.args.loss_temperature  #[b,s] [n,s] _>[b,n]
                predicts = predicts.to(torch.float64)

                batch,num = answers.shape
                answers = answers.view(-1)
                predicts = predicts.view(-1)
                pos = torch.where(answers>0.5)
                neg = torch.where(answers<0.5)
                positive_key = torch.gather(predicts,0,pos[0]).view(batch,1)
                negative_keys  = torch.gather(predicts,0,neg[0]).view(batch,num-1)
                logits = torch.cat([positive_key, negative_keys], dim=1)
                labels = torch.zeros(len(logits), dtype=torch.long).cuda()
                predicts = predicts.view(batch,num)
                answers = answers.view(batch,num)
