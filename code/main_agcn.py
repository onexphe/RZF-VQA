...
from data import fvqa_agcn as fvqa
...
class Runner:
    ...
    def train(self, epoch):
        ...
        # for question_features, answers, idx, q_len,input_ids, segment_ids, attention_mask,dep_matrix in tq:
        for idx,answers,image_batch,question_batch,bert_batch,dep_matrix in tq:
            input_ids = Variable(bert_batch[0]).cuda()
            input_mask = Variable(bert_batch[1]).cuda()
            segment_ids = Variable(bert_batch[2]).cuda()
            dep_matrix = Variable(dep_matrix).cuda()
            ...
            fusion_embedading = self.fusion_model(input_ids, segment_ids, input_mask,dep_adj_matrix=dep_matrix, dep_type_matrix=dep_matrix)

            # Classifier-based methods
            if args.method_choice == 'CLS':
                # TODO: Normalization?
                #predicts = self.answer_net(fusion_embedading)
                predicts = fusion_embedading
            ...
            # Mapping-based methods
                ...
                if self.temp:
                    print(fusion_embedading.shape,answer_embedding.shape)
                    self.temp = False
    ...
    def eval(self, epoch):
        ...
        # print(answers.data, predicts.data)
        # self._save_result(answers.data, predicts.data)
        self.val_metrics.update_per_epoch()
        if args.ZSL == 1 and not self.args.fact_map and not args.relation_map:
            self.zsl_metrics.update_per_epoch()

    def _model_choice(self, args):
        assert args.fusion_model in ['SAN', 'MLP', 'BAN', 'UD','BERT','AGCN']
        # models api
        self.fusion_model = getattr(fusion_net, args.fusion_model)(args,self.config, self.train_loader.dataset,
                                                                   self.question_word2vec)
        ...
        #     for name, p in self.fusion_model.bert.named_parameters():
        ...
        if args.method_choice != 'CLS':
            ...
            self.answer_net = getattr(answer_net, args.answer_embedding)(args, self.train_loader.dataset)
        else:
            # Classifier-based methods
            self.answer_net = SimpleClassifier(args.embedding_size, 2 * args.hidden_size, args.FVQA.max_ans, 0.5)

        # self.fusion_model = nn.DataParallel(self.fusion_model)
        self.fusion_model= self.fusion_model.cuda()

        # self.answer_net = nn.DataParallel(self.answer_net)
        self.answer_net= self.answer_net.cuda()
    ...
    def _save_result(self,data1,data2):

        data1 = data1.cpu().numpy().tolist()
        data2 = data2.cpu().numpy().tolist()
