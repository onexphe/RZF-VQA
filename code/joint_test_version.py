...
from data import fvqa_version as fvqa
from model import SimpleClassifier, Vector
from model.re_agcn.bert import BertConfig, BertModel, BertPreTrainedModel
from torchlight import (get_dump_path, initialize_exp, set_seed, show_params,
                        snapshot)
from utils import (Metrics, cosine_sim, freeze_layer, instance_bce_with_logits,
                   unseen_mask)
...
class Runner:
    def __init__(self, args):
        ...
        #bert
        self.config = BertConfig.from_json_file(os.path.join("./model/re_agcn/bert_config.json"))
...
#         self.saveResult2json()
    ...
    def eval(self):
        ...
        for visual_features, boxes, question_features, question, answers, idx,input_ids, segment_ids, attention_mask, q_len in tq:
            # if self.temp:
            #     print(visual_features.shape,boxes.shape,question_features.shape,q_len.shape)
            #     self.temp = False
            with torch.no_grad():
...
#                 fusion_embedading_rel = self.fusion_model_rel(question, q_len,input_ids, segment_ids, attention_mask)
                ...
                fusion_embedading_fact = self.fusion_model_fact(question, q_len,input_ids, segment_ids, attention_mask)
...
#                     mask_matrix = self._get_mask_on_ans_weight(predicts_ans,predicts_rel, predicts_fact, fact_relation_to_ans)
...
#                 self.explain_experiments(predicts_rel,predicts_fact,predicts_ans,predicts_ans_orig,answers,fact_relation_to_ans,idx)
    ...
    def explain_experiments(self, predicts_rel, predicts_fact, predicts_ans, predicts_ans_orig, answers, fact_relation_to_ans, idx):
        ...
        if True or self.val_metrics.correct_1 >= self.val_metrics_orig.correct_1 and self.val_metrics.correct_1 > 0:
            ...
            for it in range(predicts_rel.shape[0]):
                ...
                pre_t = []
                pre_h_r_t = []
                top_rel_num, top_rel = predicts_rel.topk(self.args.top_rel, dim=1)
                top_fact_num, top_fact = predicts_fact.topk(self.args.top_fact, dim=1)
                pre_ans_num, pre_ans = predicts_ans.data.topk(10, dim=1)
                pre_ans_orig_num, pre_ans_orig = predicts_ans_orig.data.topk(10, dim=1)
                if len(answers.data.shape) == 3:
                    answers = answers.data[it]
                _, real_ans = answers.topk(1, dim=1)

                top_rel_num = top_rel_num[it].cpu().numpy().tolist()
                top_fact_num = top_fact_num[it].cpu().numpy().tolist()
                pre_ans_num = pre_ans_num[it].cpu().numpy().tolist()
                pre_ans_orig_num = pre_ans_orig_num[it].cpu().numpy().tolist()
                        ...
                        if name in fact_relation_to_ans.keys():
                            for k in fact_relation_to_ans[name]:
                                if k in pre_ans:
                                    ...
                                    pre_h_r_t.append(name_real+'_'+self.answer_id[k])
                ...
                pre_h_r_t = list(set(pre_h_r_t))
                
                self.result[str(idx)] = [idx,real_ans,pre_ans_real,pre_ans_orig_real,top_rel_real,top_fact_real,top_rel_num,top_fact_num,pre_ans_num,pre_ans_orig_num]
                ...
                if str(idx) in ['1240','551','1434','1613','1792','1080']:
                    logger.info('#################################################################################')
                    logger.info(f'top_rel  = {top_rel }, top_fact = {top_fact}, pre_ans = {pre_ans},pre_ans_orig = {pre_ans_orig}')
                    ...
                    logger.info(f'our model predict = {pre_ans_orig_real}')
                    logger.info('#################################################################################')
    ...
    def get_entity_id(self):
        ...
        num = "3"
        test_order_data = osp.join(exp_data, "test_data", "test" + num, "all_qs_dict_release_test_500_order.json")
    ...
    def saveResult2json(self):
        with open('test.json','w') as f:
            json.dump(self.result,f, sort_keys=True, indent=4, separators=(',', ':'), ensure_ascii=False) 
    ...
    def _model_choice(self, args):
        # models api
        if args.fusion_model == 'BERT':
            fusion_model = getattr(fusion_net, args.fusion_model)(args,self.config, self.train_loader.dataset,
    ...
    def _get_mask_on_ans_weight(self,predicts_ans, predicts_rel, predicts_fact, fact_relation_to_ans):
        # self.train_loader.dataset 必须是relation和fact，这样才能映射到relation和fact上。不然都是 128 * 500
        mask_ans = torch.zeros(predicts_rel.shape[0], self.args.FVQA.max_ans)
        mask_ans_des = torch.ones(predicts_rel.shape[0], self.args.FVQA.max_ans)
        # 把每一行的 predicts_rel 和 predicts_fact 中 top 拿出来凑对并且从fact_relation_to_ans筛选，将对应位置置0。rel top10，fact top30
        _, top_rel = predicts_rel.topk(self.args.top_rel, dim=1)
        _, top_fact = predicts_fact.topk(self.args.top_fact, dim=1)
            ...
            for i in range(self.args.top_rel):
                for j in range(self.args.top_fact):
                    ...
                    if name in fact_relation_to_ans.keys():
                        for k in fact_relation_to_ans[name]:
#                             if self.temp:
#                                 print(predicts_rel[item][top_rel[item][i].item()].item(),predicts_fact[item][top_fact[item][j].item()])
#                                 self.temp = False
                            # if mask_ans[item][k] > 0.1:
                            #     tmp += 1
                            # remove the mask from those ans ...
                            # mask_ans[item][k] = predicts_rel[item][top_rel[item][i].item()].item()+predicts_fact[item][top_fact[item][j].item()]
                            # mask_ans_des[item][k] = 0

                            score = predicts_rel[item][top_rel[item][i].item()].item()*10+predicts_fact[item][top_fact[item][j].item()]/10
                            list_t.append([item,k,score,predicts_rel[item][top_rel[item][i].item()].item(),predicts_fact[item][top_fact[item][j].item()]])
            
            list_t.sort(key=sort_t)
            list_t.reverse()
            if self.temp:
                print(list_t)
                self.temp = False
        
            for one in list_t[0:6]:
                item = one[0]
                k = one[1]
                score = one[2]
                mask_ans[item][k] = score
                mask_ans_des[item][k] = 0
        ...
        mask_ans = mask_ans_des * -1 * self.args.soft_score + mask_ans * 1 * 0.1
